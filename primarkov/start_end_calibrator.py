...
class StartEndCalibrator:
    ...
    def __init__(self, cc: ParameterCarrier):
        ...
        self.distance_network = nx.DiGraph()
        self.state_number = -1
    ...
    def heap_dijkstra(self, distance_network, ga, n, source):
        dist = [inf] * (n + 1)
        st = [False] * (n + 1)
        heap = [(0, source)]  # 距离，点
        dist[source] = 0
    ...
    def heap_find_shostest_dijkstra(self, distance_network, ga, dist, source, target):
        res = [target]
        while 1:
            for j, w in ga[target]:
                if (dist[target] == dist[j] + w):
                    res.append(j)
                    target = j
                    break
            if target == source:
    ...
    def calculate_shortest_path_length(self, grid: Grid):
        print("calculate_shortest_path_length begin !!!!")
        start_state_number = self.non_zero_start_indices.size
        end_state_number = self.non_zero_end_indices.size
        print(self.non_zero_start_indices)
        print(self.non_zero_end_indices)
        ...
        m = self.distance_network.number_of_edges()
        n = self.distance_network.number_of_nodes()
        ga = [[] for _ in range(n + 1)]
        for i, edge in enumerate(self.distance_network.edges(data=True)):
            node1, node2, data = edge
            w = data['weight']
            ga[node1].append((node2, w))
            ga[node2].append((node1, w))
        j = 0
        for inner_start_index in range(start_state_number):
            j += 1
            start = time.time()
            usable_start_index = self.non_zero_start_indices[inner_start_index]
            dis = self.heap_dijkstra(self.distance_network, ga, n, source=usable_start_index)
            for inner_end_index in range(end_state_number):
                usable_end_index = self.non_zero_end_indices[inner_end_index]
                # shortest_path_1 = nx.dijkstra_path(self.distance_network, source=usable_start_index,
                    # target=usable_end_index)
                shortest_path = self.heap_find_shostest_dijkstra(self.distance_network, ga, dis, source=usable_start_index, target=usable_end_index)
                # a = [0] * len(shortest_path)
                # for i in range(len(shortest_path)):
                #     a[i] = shortest_path_1[i] - shortest_path[i]
                # print(a)
                ...
                self.inner_indices_shortest_large_cell_paths_lengths[inner_start_index, inner_end_index] = len(
                    large_cell_path)
                # print(usable_start_index, usable_end_index, end_state_number)
            end = time.time()
            print(j, end - start, start_state_number)
        self.inner_indices_arithmetic_mean_length = np.empty(self.inner_indices_shortest_path_lengths.shape)
        print("dijkstra_path answer !!!!")
        ...
        print("calculate_shortest_path_length end !!!!")
    ...
    def distribution_optimization_cvxpy2(self, loose_parameter=20):
        ...
        objective = cp.Minimize(start_error + end_error)    # 构建优化目标，将起始误差和结束误差的和最小化
        constraints = [distribution >= 0,
